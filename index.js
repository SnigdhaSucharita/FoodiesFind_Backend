...
app.get("/", (req, res) => {
  return res
    ...
    .json({
      message:
        "BD4-Assignment 1- Backend for a food discovery app called FoodieFinds.",
    });
});
...
app.get("/restaurants/details/:id", async (req, res) => {
  ...
  try {
    ...
    if (result.restaurant.length === 0) {
      return res
        .status(404)
        .json({ message: "No restaurant by this id found." });
    }
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ error: error.message });
...
app.get("/restaurants/cuisine/:cuisine", async (req, res) => {
  ...
  try {
    ...
    if (result.restaurants.length === 0) {
      return res
        .status(404)
        .json({ message: `No restaurant with ${cuisine} cuisine found.` });
    }
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ error: error.message });
...
app.get("/restaurants/filter", async (req, res) => {
  ...
  try {
    ...
    if (result.restaurants.length === 0) {
      return res
        .status(404)
        .json({ message: `No restaurant with given filter found.` });
    }
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ error: error.message });
...
app.get("/dishes/details/:id", async (req, res) => {
  ...
  try {
    ...
    if (result.dish.length === 0) {
      return res.status(404).json({ message: "No dish by this id found." });
    }
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ error: error.message });
...
app.get("/dishes/filter", async (req, res) => {
  ...
  try {
    ...
    if (result.dishes.length === 0) {
      return res
        .status(404)
        .json({ message: "No dish with this filter found." });
    }
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({ error: error.message });
